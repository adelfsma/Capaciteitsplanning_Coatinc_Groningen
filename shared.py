...
def load_published_data():
    ...
    for fname in export_files:
        ...
        export_file_summary.append({"Bronbestand": fname,"Bron_week": extract_week_label(fname),"Aantal_regels_ingelezen": len(tmp)})
    ...
    holiday_df = holiday_df.dropna(subset=["Datum"]).drop_duplicates(subset=["Datum","Omschrijving","Type"]).sort_values("Datum").reset_index(drop=True)
    return merged, pd.DataFrame(export_file_summary), "OrderExport2G.xlsx", holiday_df

def build_dashboard_data(df_raw: pd.DataFrame, holiday_df: pd.DataFrame, startdatum, capaciteit_kg: int, offset: int, kg_per_traverse: int):
    ...
    dag_orders = df_plan.groupby("Verzinkdatum", as_index=False).agg(Gewicht_kg=("Gewicht_effectief_kg","sum"), Aantal_orders_te_verzinken=("Nummer","count"))
